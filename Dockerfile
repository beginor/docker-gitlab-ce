...
#  Building Stage   #
#####################
FROM gitlab/gitlab-ce:11.0.4-ce.0 as builder
...
RUN set -xe \
    ...
    && git diff --diff-filter=d v11.0.4..v11.0.4-zh -- . ${IGNORE_DIRS} > ../zh_CN.diff \
    && echo " # Patching ..." \
    && patch -d ${GITLAB_DIR} -p1 < ../zh_CN.diff \
    && echo " # Copy assets files ..." \
    && git checkout v11.0.4-zh \
...
#  Production Stage  #
######################
FROM gitlab/gitlab-ce:11.0.4-ce.0 as production
...
ENV GITLAB_VERSION=v11.0.4
ENV GITLAB_DIR=/opt/gitlab/embedded/service/gitlab-rails
ENV GITLAB_GIT_ZH=https://gitlab.com/xhang/gitlab.git
ENV GITLAB_GIT_COMMIT_UPSTREAM=v11.0.4
ENV GITLAB_GIT_COMMIT_ZH=v11.0.4-zh
